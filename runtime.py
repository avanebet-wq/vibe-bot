...
from config import TOKEN, OPENROUTER_KEY, HF_TOKEN, HF_BASE_URL, AI_MODEL, ALLOWED_GROUPS, ALLOWED_GROUPS_RAW, DENIED_MSG, KYIV_TZ, SYS_PROMPT, SUSP, MUTES, CONFL, LOG_CHAT_ID
...
api_keys = [k.strip() for k in (OPENROUTER_KEY or "").split(",") if k.strip()]

current_key_idx = 0

key_lock = threading.Lock()

if not TOKEN or not (HF_TOKEN or api_keys):
    logging.critical("СЕКРЕТЫ НЕ НАЙДЕНЫ: нужен BOT_TOKEN и HF_TOKEN либо OPENROUTER_KEY")
    exit(1)
